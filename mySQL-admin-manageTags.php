    ...
    while( $row = $STH->fetch() ) {
      if( is_string($row['name']) AND is_string( $row['status'] ) )
         print "<tr><td>" . $row['id'] . "</td><td>" . $row['name'] . " </td><td>" . $row['status'] . "</td><td><a href=\"mySQL-admin-edit-tag.php?id=" . $row['id'] . "\">Edit</a></td><td><a href=\"mySQL-admin-delete-tag.php?id=" . $row['id'] . "\">Delete</a></td></tr>";      
     }
